...
class MimTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, mask_size, mask_type, mask_ratio, synthesize, output_folder=None,
                 dataset_directory=None, batch_dice=True, stage=None, unpack_data=True, loss_type=None,
        ...
        self.mask_type = self.mask_ratio = self.mask_size = self.synthesize = self.loss_type = None
    ...
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            if self.val_eval_criterion_MA > self.best_val_eval_criterion_MA:
                ...
                if self.save_best_checkpoint: self.save_model_naive(join(self.output_folder, "model_best.pth"))
    ...
    def gene_mask(self, x, phase, mask_seq):
        if phase is None:
            ...
            x, mask_seq = random_block_masking_generator(x, self.mask_ratio, self.mask_size, mask_seq)
            return x, mask_seq
        elif phase == 'constant_channel':
            x, mask_seq = constant_channel(x, self.mask_ratio, self.mask_size[0], mask_seq)
            return x, mask_seq
        elif phase == 'channel_wise':
            x, mask_seq = channel_wise(x, self.mask_ratio, mask_seq)
            return x, mask_seq
        else:
            raise ValueError(f'Get phase {phase}, needed to be "block_wise" "constant_channel" "channel_wise"')
        ...
        if self.fp16:
            with autocast():
                ...
                if self.loss_type == 'predict':
                    target, _ = self.gene_mask(target, self.mask_type, [1 - x for x in mask_seq])
                    output, _ = self.gene_mask(output, self.mask_type, [1 - x for x in mask_seq])
                else:
                    pass
    ...
    def validate(self, validation_folder_name: str = 'validation_raw', overwrite=True):
        ...
        for k in self.dataset_tr.keys():
            ...
            if overwrite or (not isfile(join(output_folder, fname + ".nii.gz"))):
                ...
                maybe_mkdir_p(os.path.join(self.output_folder_base, "aug_nifitis", fname))
                sitk.WriteImage(t1, os.path.join(self.output_folder_base, "aug_nifitis", fname, "t1.nii.gz"))
                sitk.WriteImage(t1_ce, os.path.join(self.output_folder_base, "aug_nifitis", fname, "t1_ce.nii.gz"))
                sitk.WriteImage(t2, os.path.join(self.output_folder_base, "aug_nifitis", fname, "t2.nii.gz"))
                sitk.WriteImage(flair, os.path.join(self.output_folder_base, "aug_nifitis", fname, "flair.nii.gz"))
